...
int main() {
  ...
  for (int i = 0; i < N_blocks; i++) {
    ...
    for (int j = 0; j < N_trows_for_block; j++) {
      ...
      sum2 = sum2 + pow(provv - 0.5, 2); // accumulazione quadrati differenza
                                         // random - 0.5 (ovvero la media)
    }
    ave_block[i] =
        sum / N_trows_for_block; // calcolo del valore medio del blocco
    ave2_block[i] = pow(ave_block[i], 2);
    delta_sigma_ave_block[i] =
        sum2 / N_trows_for_block; // array con tutte le delta sigma al quadrato
  ...
  for (int i = 0; i < N_blocks; i++) {
    ...
    sum_delta_segma_ave_prog =
        sum_delta_segma_ave_prog + delta_sigma_ave_block[i];
    sum_delta2_segma_ave_prog =
        sum_delta2_segma_ave_prog + delta2_sigma_ave_block[i];
    ...
    error_cumul_delta_sigma[i] =
        error(delta_sigma_ave_cumul[i], delta2_sigma_ave_cumul[i], i);
  }
  ...
  for (int i = 0; i < N_blocks; i++) {
    myfile << N_trows_for_block * (i + 1) << " " << ave_cumul[i] << " "
           << error_cumul[i] << endl;
  }
  myfile.close();
  ...
  for (int i = 0; i < N_blocks; i++) {
    myfile2 << N_trows_for_block * (i + 1) << " " << delta_sigma_ave_cumul[i]
            << " " << error_cumul_delta_sigma[i] << endl;
  }
  myfile2.close();

  // now i will perform a chi-squared test to verify the linear distribution in
  // this way generation of first 10^4 preudo-random number count of the number
  // in the range (0 , 1/ 100) calculating chi-squared_1 ( n_in_range - expected
  // )^2 / expected generation of second 10^4 pseudo-random number count of the
  // number in the range (1/100 , 2/ 100) calculating chi-squared_2 ( n_in_range
  // - expected )^2 / expected
  ...
  for (int i = 0; i < M_intervals; i++) {
    ...
    for (int j = 0; j < n_trows; j++) {
      ...
      if (provvisoria > double(i) / double(M_intervals) &&
          provvisoria < double(i + 1) / double(M_intervals)) {
        count++;
      }
    }

    chi_squared_i =
        pow(double(count) - avarage_intervals, 2) / avarage_intervals;
