...
//expansion of random generator with function that generate random number
//with particular distribution using the metod of inversion of the cumulative distribution
...
double Random :: CaLo(double mean , double sigma){
   // mean + sigma * tan [ pi * ( p - 0.5 )]
   //where p is the random number;

   return mean + sigma * tan( M_PI * ( Rannyu() - 0.5 ) ) ;
}
